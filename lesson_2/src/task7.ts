    ...
    //Крок 1. Введення необхідних даних
        ...
        const randomMonthNumber:number=1 + Math.floor(Math.random()*12)
        const randomDayNumber:number=Math.floor(Math.random()*7)
    
    //Крок 2. Обчислення результатів
        const sumRandomNumbers:number = randomMonthNumber+randomDayNumber
